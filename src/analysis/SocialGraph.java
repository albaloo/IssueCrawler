...
public class SocialGraph {
	...
	public String[] designers = { "yoroy", "Roy", "Bojhan", "bojhan", "leisa",
			"leisareichelt", "cliff", "mark", "Mark" };
	...
	public void create(ArrayList<IssueInfo> issueInfos) {
		...
		for (IssueInfo issueInfo : issueInfos) {
			String issueName = (issueInfo.getLink().replaceFirst("node", ""))
					.substring(2);
			Node issueNode = new Node(Node.THREAD, issueName,
					issueInfo.getLink(), issueInfo.getNumComments(),
					issueInfo.isConsensus());
	...
	private IssueAnalysis createIssueAnalysis(IssueInfo issueInfo) {
		...
		for (int i = 0; i < issueInfo.getComments().size(); i++) {
			if (issueInfo.getComments().get(i).getAuthor() == null) {
				...
				Double duration = new Double(Stats.findDuration(
						prevCommentTime, currCommentTime));
				if (duration > 0)
					durations.add(duration);
				else if (durations.size() > 0)
					durations.set(durations.size() - 1,
							duration + durations.get(durations.size() - 1));

				// look for the node
				for (int j = 0; j < tempAuthorNodes.size(); j++) {
					if (tempAuthorNodes.get(j).getName()
							.equals(issueInfo.getComments().get(i).getAuthor())) {
						currentAuthorsNode = tempAuthorNodes.get(j);
						break;
					}
				}

				if (currentAuthorsNode == null
						|| currentAuthorsNode.equals(null)) {
					currentAuthorsNode = new Node(Node.AUTHOR, issueInfo
							.getComments().get(i).getAuthor(), issueInfo
							.getComments().get(i).getAuthorLink(), 1, false);
					tempAuthorNodes.add(currentAuthorsNode);
				} else {
					currentAuthorsNode.addNumComments(1);
				}

				currentAuthorsNode.updateRepliersAndThreads(issueInfo,
						issueInfo.getComments().get(i),
						Stats.findDuration(prevCommentTime, currCommentTime));
				numReplyTo += issueInfo.getComments().get(i).getReceiverNames()
						.size();
				// receivers.addAll(findUniqueReceivers(issueInfo.getComments().get(i).getReceiverNames(),
				// receivers));

				prevCommentTime = currCommentTime;

				String currentContent = issueInfo.getComments().get(i)
						.getContent();
				if (currentContent != null) {
					sumNumWords += ContentAnalysis
							.calculateNumWords(currentContent);
					...
					double positiveRatio = numPositiveWords
							/ (sumNumWords - numStopWords);
					double negativeRatio = numNegativeWords
							/ (sumNumWords - numStopWords);
		...
		double replyToRatio = (double) numReplyTo;
	...
	private void addNodesToSocialGraph(ArrayList<Node> tempAuthorNodes) {
		...
		for (Node tempAuthorNode : tempAuthorNodes) {
			...
			if (!authorFound) {
				socialGraphNodes.put(tempAuthorNode.getName().toLowerCase(),
						tempAuthorNode);
		...
		for (Iterator<String> i = keyNodes.iterator(); i.hasNext();) {
			...
			if (node.getRepliers().size() != 0) {
				...
				for (Iterator<String> iter = keys.iterator(); iter.hasNext();) {
					...
					SocialMatrixCell value = (SocialMatrixCell) node
							.getRepliers().get(key);

					Node findNode = socialGraphNodes.get(key.toLowerCase());
					if (findNode == null)
						System.out.println("fill Social Matrix: Errrrrrror: "
								+ key);
					else {
						int index = findNode.getIndex();
						socialMatrix[node.getIndex()][index].addComments(value
								.getComments());
			...
			if (node.getThreads().size() != 0) {
				...
				for (Iterator<String> iter = keys.iterator(); iter.hasNext();) {
					...
					if (findNode == null)
						System.out
								.println("fill Social Matrix, threads: Errrrrrror: "
										+ key);
					else {
	...
	public void fillSocialGraph() {
		...
		for (Iterator<String> i = keyNodes1.iterator(); i.hasNext();) {
			...
			for (Iterator<String> j = keyNodes2.iterator(); j.hasNext();) {
				...
				Double value = (double) socialMatrix[node1.getIndex()][node2
						.getIndex()].calculateValue()
						/ (double) node1.getOutgoingEdges();
				if (value > 0)
					socialGraph.addEdge(
							new Edge(value,
									socialMatrix[node1.getIndex()][node2
	...
	public double findMeanNumConsensusThreads(IssueInfo issueInfo) {
		...
		for (int i = 0; i < issueInfo.getComments().size(); i++) {
			...
			firstNode = null;
			if (issueInfo.getComments().get(i).getAuthor() != null)
				firstNode = socialGraphNodes.get(issueInfo.getComments().get(i)
						.getAuthor().toLowerCase());
			if (firstNode == null)
				System.out.println("getMeanNumConsensus: Errrrrrror: "
						+ issueInfo.getComments().get(i).getAuthor());
			else
				firstIndex = firstNode.getIndex();
			if (firstIndex != -1)
				for (int j = 0; j < socialMatrix.length; j++)
					result += socialMatrix[firstIndex][j]
							.getNumConsensusThreads(issueInfo.getComments()
									.get(i).getDate());
		}
	...
	public int findNumPreviousComments(UserProfileInfo authorInfo) {
		...
		if (authorInfo.getUserName() != null)
			firstNode = socialGraphNodes.get(authorInfo.getUserName()
					.toLowerCase());
		if (firstNode == null)
			System.out.println("findNumComments: Errrrrrror: "
	...
	public String findAuthorsInfo(IssueInfo issueInfo) {
		...
		for (int i = 0; i < issueInfo.getComments().size(); i++) {
			...
			if (issueInfo.getComments().get(i).getAuthor() != null) {
				...
				for (int j = 0; j < tempAuthors.size(); j++) {
					if (tempAuthors.get(j).getUserName()
							.equals(issueInfo.getComments().get(i).getAuthor())) {
						currentAuthorsInfo = tempAuthors.get(j);
						break;
					}
				}
				if (currentAuthorsInfo == null
						|| currentAuthorsInfo.equals(null)) {
					numUniqueAuthors++;
					currentAuthorsInfo = new UserProfileInfo(issueInfo
							.getComments().get(i).getAuthor(), issueInfo
							.getComments().get(i).getAuthorLink());
					tempAuthors.add(currentAuthorsInfo);
				}
				currentAuthorsInfo.addComments();
				currentAuthorsInfo.addDate(issueInfo.getComments().get(i)
						.getDate());
				currentAuthorsInfo.setLongestThread(issueInfo.getComments()
						.size());
			}
		}
		...
		for (UserProfileInfo tempAuthor : tempAuthors) {
			...
			for (int k = 0; k < authorsInfos.size(); k++) {
				if (authorsInfos.get(k).getUserName()
						.equals(tempAuthor.getUserName())) {
					authorsInfos.get(k).setComments(
							authorsInfos.get(k).getComments()
									+ tempAuthor.getComments());
					authorsInfos.get(k).addThreads();
					authorsInfos.get(k).addDates(tempAuthor.getDates());
					authorsInfos.get(k).setLongestThread(
							tempAuthor.getLongestThread());
					authorFound = true;
					if (authorsInfos.get(k).hasInterest("usability"))
						usabilityInterestNum++;
					authorNames.add(authorsInfos.get(k));
					counter++;
					membershipWeeks.add(new Integer(authorsInfos.get(k)
							.getMembershipWeeks()));
					if (!tempAuthor.getUserName().equals("System Message"))
						expertise.addAll(findExpertise(authorsInfos.get(k)));
					previousComments.add(findNumPreviousComments(authorsInfos
							.get(k)));
					pageRanks.add(socialGraphNodes.get(
							authorsInfos.get(k).getUserName().toLowerCase())
							.getRank());
					break;
				}
			}
			if (!authorFound) {
				...
				membershipWeeks
						.add(new Integer(tempAuthor.getMembershipWeeks()));
				if (!tempAuthor.getUserName().equals("System Message"))
					expertise.addAll(findExpertise(tempAuthor));
				pageRanks.add(socialGraphNodes.get(
						tempAuthor.getUserName().toLowerCase()).getRank());
			}
		}
		...
		for (int i = 0; i < authorNames.size(); i++) {
			for (int j = i + 1; j < authorNames.size(); j++) {
				for (int k = j + 1; k < authorNames.size(); k++) {
					int firstIndex = socialGraphNodes.get(
							authorNames.get(i).getUserName().toLowerCase())
							.getIndex();
					int secondIndex = socialGraphNodes.get(
							authorNames.get(j).getUserName().toLowerCase())
							.getIndex();
					int thirdIndex = socialGraphNodes.get(
							authorNames.get(k).getUserName().toLowerCase())
							.getIndex();
					Date firstDateOfIssue = issueInfo.getComments().get(0)
							.getDate();
					// && authorNames.get(i).sharedInterest(authorNames.get(j))
					if (((!socialMatrix[firstIndex][secondIndex]
							.isEmpty(firstDateOfIssue) || !socialMatrix[secondIndex][firstIndex]
							.isEmpty(firstDateOfIssue)))
							&& ((!socialMatrix[secondIndex][thirdIndex]
									.isEmpty(firstDateOfIssue) || !socialMatrix[thirdIndex][secondIndex]
									.isEmpty(firstDateOfIssue)))
							&& ((!socialMatrix[firstIndex][thirdIndex]
									.isEmpty(firstDateOfIssue) || !socialMatrix[thirdIndex][firstIndex]
									.isEmpty(firstDateOfIssue))))
		...
		double meanPreviousCommentsNum = Stats.findMean(previousComments,
				counter);
		meanPreviousComments += Double.toString(meanPreviousCommentsNum);
		double meanMembershipWeeksNum = Stats
				.findMean(membershipWeeks, counter);
		...
		String result = numUniqueAuthors + "\t" + usabilityInterest + "\t"
				+ triangularConnections + "\t" + meanPreviousComments + "\t"
				+ meanMembershipWeeks + "\t" + meanPageRank + "\t"
				+ currCommentsOfCreator + "\t" + meanExpertise;
	...
	private ArrayList<Integer> findExpertise(UserProfileInfo authorInfo) {
		...
		for (int i = 0; i < authorInfo.getDates().size(); i++) {
			double duration = currentDate.getTime()
					- authorInfo.getDates().get(0).getTime();
